...
- 복잡한 케이스 검증은 테스트 파일 추가 대신 로직 파일에 `console.log`를 넣고 `bbd_sb.ts` 시뮬레이션 실행으로 확인한다.
- 로직 로그는 환경 변수로 토글한다. (예: `BBD_SB_LOG=1 npx ts-node ./src/slot_simulation/bbd_sb.ts 1000 --rtp=HIGH --level=1 -t 1`)
- 추가 검증이 필요할 때는 `BBD_SB_EDGE_LOG=1` (요약), `BBD_SB_LOG=1` (상세), `BBD_SB_TRACE_LOG=1` (증명 로그) 중 하나와 `SPIN_LIMIT_LOG=1`로 `bbd_sb.ts`를 길게 돌리고 스핀 제한, LIFE cap 카운트, 골든휠+라이프 동시 등장, 마지막 스핀 트리거, 스티키 해제를 확인한다.
- Barbarian Destroyer 슈퍼보너스 사양 변경 시 `games/barbarian_destroyer_super_bonus_par_sheet.xlsx` 및 `games/barbarian_destroyer_super_bonus_par_sheet.md`를 함께 수정한다.
- Barbarian Destroyer 슈퍼보너스는 LIFE = +1 (최대 3, 초과분 변환 없음)이며 LIFE 심볼도 현금 프라이즈를 가진다.
- L3 same-index는 단일 스트립에서 3연속 심볼로 3x2를 구성하고, 좌/우 컬럼은 동일 인덱스로 고정한다.
- 가중치/리스트는 숫자 키 오브젝트 대신 배열(이중 배열 포함)을 사용한다.
